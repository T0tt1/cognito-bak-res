...
export const argv = yargs
  ...
  .command(
    "backup",
    dimmed`Backup/export all users in specified user pool`,
    (yargs) => {
  ...
  .command(
    "restore",
    dimmed`Restore/import users to a single user pool`,
    (yargs) => {
      return yargs.options({
        mode: {
          default: "restore",
  ...
  .example(
    "$0 backup",
    greyed`Follow the interactive guide to backup userpool(s)`
  )
  .example(
    "$0 restore",
    greyed`Follow the interactive guide to restore userpool`
  )
  .example(
    "$0 backup -p <PROFILE> [OPTIONS]",
    greyed`Backup using the options provided`
  )
  .example(
    "$0 restore -p <PROFILE> [OPTIONS]",
    greyed`Restore using the options provided`
  )
  ...
  .option("sessionToken", {
    alias: ["st"],
    describe: dimmed`The AWS SessionToken to use. Overrides config/env settings`,
    string: true,
  })
  .option("delay", {
